...
import { generateBookFiles, loadBook } from '../kg/kg-new.js';
...
program
  ...
  .action(async (project, options) => {
    ...
    const result = generateBookFiles(projectName, worldFile);
    
    console.log(chalk.green('✓ 知识图谱已构建（独立文件）'));
    console.log(chalk.yellow(`  文风: ${result.styles.length}个`));
    console.log(chalk.yellow(`  设定: ${result.settings.length}个`));
    console.log(chalk.yellow(`  章节: ${result.chapters.length}个`));
...
function findProjectDir(projectName) {
  ...
  // Check books/ directory first (new format)
  const booksDir = path.join(process.cwd(), 'books');
  if (fs.existsSync(booksDir)) {
    try {
      const booksDirItems = fs.readdirSync(booksDir).filter(item => {
        ...
        return fs.statSync(itemPath).isDirectory() && 
               (fs.existsSync(path.join(itemPath, '文风库')) || fs.existsSync(path.join(itemPath, '大纲库')));
      });
...
program
  ...
  .action(async (target, project, options) => {
    ...
    const styleDir = path.join(projectDir, '文风库');
    const settingDir = path.join(projectDir, '设定库');
    const outlineDir = path.join(projectDir, '大纲库');
    const chapterDir = path.join(projectDir, 'chapters');
    
    if (!fs.existsSync(kgDir) && !fs.existsSync(styleDir) && !fs.existsSync(settingDir) && !fs.existsSync(outlineDir)) {
    ...
    const runner = new Runner(projectDir, projectName);
    await runner.init();
  ...
  let result;
  try {
    result = await runner.run(chapterNum);
  } catch (e) {
    console.log(chalk.red('生成出错: ' + e.message));
    return;
  }
  
  if (!result?.writer?.draft) {
  ...
  console.log(chalk.gray(`  字数: ${result.writer?.draft?.length || 0}`));
  console.log(chalk.gray(`  审计: ${result.auditor?.passed ? '通过' : '需修订'}`));
    ...
    fs.writeFileSync(chapterFile, result.writer?.draft || '');
...
program
  ...
  .action(async (target, project, options) => {
    ...
    const planner = new PlannerAgent(projectDir, runner.kg.outline);
    const plannerResult = await planner.run(chapterNum);
    
    const { ComposerAgent } = await import('../agents/composer.js');
    const composer = new ComposerAgent(projectDir, runner.kg.styles, runner.kg.settings);
    ...
    const writer = new WriterAgent(async (msgs, opts) => {
      const result = await chat(msgs, opts);
      return result;
    });
    
    const { TemplateFactory } = await import('../llm/conversation-builder.js');
    const { title, goal, plot } = writer.parseOutline(plannerResult.chapterContent);
    
    const settingsStr = Object.entries(composerResult.relevantSettings || {})
      .map(([cat, items]) => {
    ...
    const template = TemplateFactory.createWriterTemplate('openai');
    template.setContext({
      ...
      previous: '无',
      state: '无',
      constitution: runner.kg.constitution || '',
      chapterNum: chapterNum
    });
    
    const messages = template.buildInitialMessages();
...
program
  ...
  .action(async (project) => {
    ...
    // Check new format (books directory)
    const styleDir = path.join(projectDir, '文风库');
    const settingDir = path.join(projectDir, '设定库');
    const outlineDir = path.join(projectDir, '大纲库');
    
    if (fs.existsSync(styleDir) || fs.existsSync(settingDir) || fs.existsSync(outlineDir)) {
      const styles = fs.existsSync(styleDir) ? fs.readdirSync(styleDir).filter(f => f.endsWith('.md')).length : 0;
      const settings = fs.existsSync(settingDir) ? fs.readdirSync(settingDir).filter(f => f.endsWith('.md')).length : 0;
      const outlines = fs.existsSync(outlineDir) ? fs.readdirSync(outlineDir).filter(f => f.endsWith('.md')).length : 0;
      console.log(chalk.green('✓ 知识图谱: 已构建（新格式）'));
      console.log(chalk.gray(`  文风: ${styles}个, 设定: ${settings}个, 大纲: ${outlines}个`));
    } else if (fs.existsSync(kgDir)) {
