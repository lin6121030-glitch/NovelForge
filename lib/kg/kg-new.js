...
export function loadBook(bookName) {
  const constitutionPath = path.join(getBookDir(bookName), '全书宪法.md');
...
function parseRawContent(content) {
  const result = { styles: [], settings: [], chapters: [], constitution: '' };
  
  // 全书创作宪法
  const constitutionMatch = content.match(/==全书创作宪法start==([\s\S]*?)==全书创作宪法end==/);
  if (constitutionMatch) {
    result.constitution = constitutionMatch[1].trim();
  }
...
export function generateBookFiles(bookName, sourceFilePath) {
  ...
  const { styles, settings, chapters, constitution } = parsed;
  ...
  // 生成宪法文件
  if (constitution) {
    fs.writeFileSync(path.join(getBookDir(bookName), '全书宪法.md'), constitution);
  }
