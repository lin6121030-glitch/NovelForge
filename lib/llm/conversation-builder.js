...
export class ConversationExecutor {
  ...
  async executeTemplate(template, options = {}) {
    const messages = [];
    const responseIndices = template.getResponseRequiredIndices();
    let responseIndex = 0;
    
    // 构建初始消息
    const initialMessages = template.buildInitialMessages(this.context);
    ...
    return {
      ...
      finalResponse: this.history[this.history.length - 1]?.assistantReply || ''
    };
  }
}
...
export class TemplateFactory {
  ...
  static createWriterTemplate() {
    return new ConversationBuilder()
      .system('你是一个专业的小说作家，正在参与一个结构化的对话来接收所有必要的上下文信息。', { name: '系统设定' })
      
      // 角色定义
      .user('你好！我需要你帮我创作小说内容。作为专业小说作家，你具备以下能力：\n- 创作引人入胜的叙事内容和生动描述\n- 塑造角色声音和对话\n- 保持与既定世界观的一致性\n- 遵循特定的风格指南和约束\n- 创作符合章节大纲的内容\n\n请确认你理解了这个角色。', { name: '角色定义' })
      .expectResponse()
      
      // 角色信息
      .userTemplate('以下是本故事中的角色信息：\n\n{{characters}}\n\n请查看并确认你理解了这些角色。', ['characters'], { name: '角色信息' })
      .expectResponse()
      
      // 世界观信息
      .userTemplate('以下是这个世界观的相关信息：\n\n## 组织势力\n{{factions}}\n\n## 地理环境\n{{locations}}\n\n## 世界观规则\n{{worldRules}}\n\n请查看并确认你理解了这个世界设定。', ['factions', 'locations', 'worldRules'], { name: '世界观信息' })
      .expectResponse()
      ...
      // 故事大纲
      .userTemplate('以下是故事大纲：\n\n{{outline}}\n\n请查看整体故事结构。', ['outline'], { name: '故事大纲' })
      .expectResponse()
      
      // 当前章节信息
      .userTemplate('以下是当前需要创作的章节信息：\n\n{{chapterInfo}}\n\n这是你需要专注的具体范围和内容。', ['chapterInfo'], { name: '章节信息' })
      .expectResponse()
      
      // 创作任务
      .user('基于以上提供的所有信息，请创作指定章节的小说内容。\n\n要求：\n- 使用第三人称视角，紧贴主角\n- 语言白话幽默，有网感\n- 每章要有笑点或爽点\n- 禁止输出任何分析、清单、表格、说明文字\n- 只输出小说正文\n- 遵循章节大纲，保持与既定角色和世界观的一致性\n\n请现在开始创作完整的小说内容。', { name: '创作任务' })
      .expectResponse()
