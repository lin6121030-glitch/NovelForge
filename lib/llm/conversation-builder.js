...
export class ConversationTemplate {
  ...
  buildInitialMessages(context = {}) {
    ...
    for (const round of this.rounds) {
      const message = round.buildMessage(ctx);
...
/**
   ...
   * 添加系统消息模板
   */
  systemTemplate(template, dataKeys = [], options = {}) {
    this.rounds.push(new ConversationRound({
      role: 'system',
      content: template,
      dataKeys,
...
export class TemplateFactory {
  /**
   * 创建Writer Agent模板
   * @param {string} adapterType - 'minimax' | 'openai'
   */
  static createWriterTemplate(adapterType = 'openai') {
    return new ConversationBuilder()
      .systemTemplate('你是一个专业的小说作家。请严格遵守全书创作宪法创作第{{chapterNum}}章。\n\n【全书宪法】\n{{constitution}}', ['chapterNum', 'constitution'], { name: '系统设定' })
      
.userTemplate(`## 文风要求
请严格模仿以下段落的文风、节奏和叙事手法进行创作：
{{style}}
...
- 前情提要：{{previous}}
- 状态备忘：{{state}}`, 
        ['style', 'title', 'goal', 'plot', 'settings', 'previous', 'state'], 
        { name: '上下文信息' })
      
      .userTemplate('请基于以上信息创作第{{chapterNum}}章，保持与设定和数值的一致性。', ['chapterNum'], { name: '创作任务' })
