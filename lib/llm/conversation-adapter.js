...
export class EnhancedConversationExecutor {
  ...
  buildStandardMessages(template, context) {
    ...
    // 构建初始消息
    const initialMessages = template.buildInitialMessages ? template.buildInitialMessages(context) : [];
    ...
    if (template && template.rounds) {
      for (let i = 0; i < template.rounds.length; i++) {
        ...
        // 如果是模板，使用适配器构建消息
        if (round.isTemplate && round.dataKeys) {
          const adapterMessages = this.adapter.buildMessages([round], context);
