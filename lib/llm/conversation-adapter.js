...
export class AdapterFactory {
  static create(config) {
    const apiMode = config.apiMode || config.useChat;
    
    // 根据API模式选择适配器
    if (apiMode === 'MINIMAX' || apiMode === 'minimax') {
      return new MiniMaxAdapter(config);
    }
    
    // OPENAI 或 SMART 模式使用 OpenAI 适配器
...
export class EnhancedConversationExecutor {
  ...
  async executeTemplate(template, options = {}) {
    ...
    const isMiniMax = this.adapter.provider === 'minimax';
    ...
    // 根据适配器选择不同的调用方式
    if (isMiniMax) {
      // MiniMax 使用独立的 Provider，不干扰 OpenAI 流程
      const minimaxProvider = new MiniMaxProvider();
      response = await minimaxProvider.chat(messages, {
  ...
  buildStandardMessages(template, context) {
    if (template.buildInitialMessages) {
      return template.buildInitialMessages(context);
    }
